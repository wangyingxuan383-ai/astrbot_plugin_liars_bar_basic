...
AstrBot QQ 群聊《骗子酒馆基础版》插件（3~5 人，支持 AI 玩家补位）。

## 功能概览
- 群聊开房/加入/加AI/减AI/开始/质疑/剪线/状态/结束/帮助
- 私聊查看手牌、私聊暗出多牌（`/酒馆 出 2 4 5`）
- 一人一房约束（同一玩家同一时间仅可在一个房间）
- 出牌超时淘汰、剪线超时自动剪线
- AI 自动回合：固定 Provider 决策，失败自动规则兜底
- 本地素材卡牌与炸弹图（不依赖在线文生图）

## 基础规则（当前实现）
- 每小局随机目标牌：太阳 / 月亮 / 星星。
- 每小局会先清空上局手牌，再给当前存活玩家每人固定发 5 张。
- 本大局总牌池在开局时锁定：`3/4 人 = 20 张（6/6/6/2），5 人 = 27 张（8/8/8/3）`。
...
## AI 玩家机制
- AI 与真人共用同一状态机与结算逻辑，不走独立并行游戏逻辑。
- 房主仅可在等待阶段调整 AI：
- `/酒馆 加AI [数量]`
- `/酒馆 减AI [数量]`
- 人数约束：真人+AI 总人数保持 3~5，且至少 1 名人类玩家。
- AI 行动策略：
- 优先调用固定 `ai_provider_id`
- Provider 未配置/失效/超时/解析失败时，自动降级到本地规则 AI
- AI 仅基于公开局面 + 自己手牌决策，不读取其他玩家暗牌。
- AI 在群内立即行动，不等待额外指令；每次动作默认都会有短台词（可配置开关/概率）。
...
- `/酒馆 加入`：加入本群房间。
- `/酒馆 加AI [数量]`：仅房主，等待阶段可添加 AI（默认 1）。
- `/酒馆 减AI [数量]`：仅房主，等待阶段可移除 AI（默认 1）。
- `/酒馆 开始`：房主开局（要求总人数 3~5 且至少 1 名人类）。
...
### 私聊限制
- 私聊不能使用开房/加入/加AI/减AI/开始/质疑/剪线/结束等群聊指令。
...
3. 房主可选：`/酒馆 加AI 1`
4. 房主群里：`/酒馆 开始`
5. 当前真人行动玩家私聊：`/酒馆 手牌` -> `/酒馆 出 序号...`
6. 群里：下一家选择质疑或继续轮转（若为 AI，则系统自动执行）
7. 进入惩罚时：受罚玩家群里 `/酒馆 剪线 红|蓝|黄`（若受罚者为 AI，则系统自动剪线）
...
- `require_dm_reachable_before_start`：开局前检查私聊可达（仅检查真人）
- `room_ttl_minutes`：等待阶段房间自动回收分钟数
- `ai_enabled`：是否启用 AI 玩家功能
- `ai_provider_id`：AI 固定 Provider ID（留空则直接规则 AI）
- `ai_llm_timeout_seconds`：AI 调用 Provider 超时时间
- `ai_llm_retry_times`：AI 调用失败重试次数
- `ai_taunt_enabled`：是否启用 AI 简短台词
- `ai_taunt_probability`：AI 使用“完整台词池”的概率（0~1，默认 1；未命中仍发送简短台词）
- `ai_max_play_cards`：AI 单次最多出牌数（不影响人类）
...
- AI 行动通过房间级任务调度，按 `action_token` 做幂等防重入。
...
## 目录结构
- `main.py`：核心逻辑（状态机、命令、超时、AI 调度、结算）
